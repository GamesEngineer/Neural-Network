...
public static class Tensor
{
    ...
    /// <param name="kernels">Array of kernel filters [channels, depth, kernel size, kernel size]</param>
        ...
        for (int z = 0; z < depth; z++)
        {
            for (int kernelY = 0; kernelY < kernelSize; kernelY++)
            {
                ...
                for (int kernelX = 0; kernelX < kernelSize; kernelX++)
                {
                    ...
                    float w = kernels[channelIndex, z, kernelY, kernelX];
    ...
    /// <param name="column">Index of column in the OUTPUT tensor</param>
    /// <param name="row">Index of row in the OUTPUT tensor</param>
    /// <param name="inChannelIndex">Index of the kernel filter to use</param>
    ...
    public static float Convolution(int column, int row, int inChannelIndex, float[,,,] kernels, float[,,] tensor, int stride)
    {
        ...
        for (int z = 0; z < depth; z++)
        {
            for (int n = 0; n < kernelSize; n++)
            {
                ...
                for (int m = 0; m < kernelSize; m++)
                {
                    ...
                    float w = kernels[z, inChannelIndex, kernelY, kernelX];
